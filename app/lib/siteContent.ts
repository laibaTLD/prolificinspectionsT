...
function getPublishedHeaderPages(pages?: Page[]): Page[] {
  return (
    pages
      ?.filter((p) => p.status === 'published' && p.name?.trim())
      .sort(
        (a, b) =>
          ((a as Page & { order?: number }).order ?? 0) -
          ((b as Page & { order?: number }).order ?? 0)
...
export function getHeaderNavLinks(pages?: Page[]): {
  mainNavLinks: HeaderNavLink[];
  mobileNavLinks: HeaderNavLink[];
  contactNav: HeaderNavLink;
} {
